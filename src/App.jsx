...
async function validatePhoto(base64Data) {
  ...
  try {
    const result = await callClaude([{
      ...
      content: [
        ...
        { type: "text", text: "这张照片是人体身材照片吗？身材照片应该能看到身体的大部分，用于健身体型分析。请只回答：YES（是身材照）或 NO（不是身材照，例如脸部特写、风景、食物、动物等）。只回答一个词。" }
      ]
    }]);
    return result.trim().toUpperCase().includes("YES");
  } catch {
    return true; // if validation fails, allow through
  }
}
...
sets必须是数字`;
      ...
      // Build message with optional photo
      let userMessage;
      if (u.photo) {
        const mediaType = u.photo.split(";")[0].split(":")[1] || "image/jpeg";
        const base64 = u.photo.split(",")[1];
        userMessage = {
          role: "user",
          content: [
            { type: "image", source: { type: "base64", media_type: mediaType, data: base64 } },
